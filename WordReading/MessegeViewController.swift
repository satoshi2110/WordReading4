...
class MessegeViewController :UIViewController {
    ...
    override func viewDidLoad() {
        ...
        使い方
        　３つの点から１つ選ぶと、単語がランダムに表示されます。ひらがなが1文字ずつ表示されるので、お子さまにタップさせてください。タップするたびに対応する音声が流れます。すべての文字（例：あめ）が表示された後、イラストが表示され、一部のイラストでは音も鳴ります。新しい単語が表示され、この流れを４〜５回繰り返します。
        ...
        テスト
        　応用行動分析学の「見本合わせ課題手続き」を取り入れ、絵に対応する文字を選択することで、絵と文字の関係を学習できる仕組みになっています。お子さまの習熟度に応じて、テストを先に実施しても構いません。
        
        基礎：継次的刺激ペアリング手続きで使用したひらがなの読みを確認します。
        応用：未使用の単語を用いて、ひらがなの読みを確認します。
