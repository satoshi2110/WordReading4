...
class MessegeViewController :UIViewController {
    ...
    override func viewDidLoad() {
        ...
        使い方
        　2～4文字のひらがなをお子さまにタップさせてください。ひらがなが1文字ずつ表示され、タップするたびに対応する音声が流れます。すべての文字が表示された後、イラストが提示されます。
        　繰り返し見ることで、文字と音声、絵の関係を無理なく習得できます。このアプリでは、「を」以外のひらがな50音を学習できます。

        テスト
        　基礎テスト：継次的刺激ペアリングで使用したひらがなの読みを確認します。
        　応用テスト：未使用の文字を用いて、ひらがなの読みを確認します。
        　お子さまが正しく選べるようになったら、「もっと早く選んでみよう！」と声をかけてみてください。選択スピードが上がることで記憶が定着しやすくなり、アプリ以外の場面でも読める機会が増えていきます。

        　テスト履歴では、苦手な単語を確認できます（記録は1年後に自動的に消去されます）。
